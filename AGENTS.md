...
- Treat generated Game Boy/NES sample ROMs and NES runtime ABI sidecars as tracked artifacts. Regenerate them deliberately when their source sample or generation code changes, and before every push, so the repository copy matches the compiler.
...
3. Run the relevant validation before each merge (`dotnet test RetroSharp.sln -m:1`, `git diff --check`, and regenerate tracked ROMs when their source or generation code changed).
