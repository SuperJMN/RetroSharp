...
| Background blocks tear/glitch only while crossing a tile boundary (especially on heavy frames such as landing on a platform) | Camera tile streaming writing VRAM during active display or trying to stream too many rows in one VBlank; confirm the VBlank gate (`EmitWaitForVBlankBeforeStream`, `LY >= 144`) runs before the column/row writes and that horizontal streaming is capped to visible rows. Read `0xFF44` directly for `LY`; some debug bridges report a stale `LY` in compact PPU state. |
