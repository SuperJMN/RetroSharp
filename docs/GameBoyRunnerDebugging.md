...
| Scroll or player movement stutters/judders — motion is jerky though the pixels themselves are clean | Frame-pacing/cadence, not a VRAM glitch. This is usually a **confirmed report** (see the Acceptance Policy in `AGENTS.md`): reproduce it on physical playback (PyBoy/RetroArch/hardware) and treat that scene as the acceptance observer even if the in-process cadence harness reports clean deltas. Check that camera scroll advances a **fixed** pixels-per-displayed-frame: variable per-frame work (row/column streaming, collision, audio service) that overruns VBlank drops or duplicates displayed frames; confirm `Input.Poll()` and the game tick run exactly once per frame, that `Camera.Apply()` writes `SCX`/`SCY` every frame at a stable VBlank edge, and that scroll velocity plus streaming amortize evenly rather than bursting on tile-boundary frames. Isolate with `02-flat-ground-camera` (camera cadence alone) before blaming full-scene load in `04`. |
...
- Runtime and editable map data: complete `samples/runner/assets/maps/stage1.tmx`; editable tileset: `stage1.tsx`. The older `stage1.playable.tmj` crop is no longer the runner input.
