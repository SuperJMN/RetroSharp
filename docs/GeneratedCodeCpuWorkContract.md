...
| --- | --- | --- | --- |
| `actor.spawn.recycle` / `actor.spawn.scan` | Literal pool/spawn visit counts from generated `for` bounds. NES code-banked builds also report a conservative `actor.spawn.scan` video-safe upper when an outlined generated spawn activation call appears between `Video.WaitVBlank()` and `Camera.Apply()`, because that source order can delay the video-safe publication path. | Target/profile lower and upper cycles per complete traversal, including loop control and active/used branch paths. | General actor-spawn paths still need full calibration; the NES prefix upper is deliberately one-sided and only prevents the video-safe report from hiding an outlined body behind a `JSR`. |
