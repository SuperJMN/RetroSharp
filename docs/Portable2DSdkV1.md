...
The current source spelling is `import RetroSharp.Portable2D;` followed by SDK dot-call forms such as `Video.WaitVBlank()` and `Camera.SetPosition(x, y)`. Game Boy and NES still auto-import `RetroSharp.Portable2D` for existing samples by default, but explicit import is the forward-compatible source shape and unknown imports are rejected. Hosts can compile with `SdkLibraryImportMode.ExplicitOnly` to disable that legacy autoimport; in that mode SDK module calls require an imported SDK library, while source that does not use the SDK can still compile. Additional source-level SDK libraries can be supplied through `SdkLibraryRegistry`. The CLI exposes the same MVP resolver through repeated `--lib-path <path>` options. Each path may be a package directory containing `retrosharp-library.json`, or a directory whose direct children are package directories. A manifest declares the import path, source files, and optional target ids:
