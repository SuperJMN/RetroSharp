...
Projects can load `RetroSharp.Portable2D` from their manifest `libraries` list, then use SDK dot-call forms such as `Video.WaitVBlank()` and `Camera.SetPosition(x, y)` from source without a code-file `import`. Game Boy and NES still auto-import `RetroSharp.Portable2D` for existing samples by default, and `import RetroSharp.Portable2D;` remains accepted as the explicit transitional source-level form. Unknown imports are rejected. Hosts can compile with `SdkLibraryImportMode.ExplicitOnly` to disable that legacy autoimport; in that mode SDK module calls require either an imported SDK library or a library supplied by the host/project, while source that does not use the SDK can still compile. Additional source-level SDK libraries can be supplied through `SdkLibraryRegistry`. The CLI exposes the same MVP resolver through repeated `--lib-path <path>` options plus project-manifest `libraryPaths` and `libraries` fields: paths discover packages, while `libraries` names the import paths to load. Each path may be a package directory containing `retrosharp-library.json`, or a directory whose direct children are package directories. A library manifest declares the import path, source files, and optional target ids:
