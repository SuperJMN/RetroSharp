...
| `pool.Update()` | Move active projectiles horizontally by their definition `speedX` in the per-projectile travel direction (rightward for `direction == 0`, leftward otherwise) and vertically by `speedY`, increment age, and deactivate when age reaches `lifetime`. `GravityArc` currently increments the stored vertical velocity field for future behavior expansion. |
