...
`sources` are source-only RetroSharp files loaded relative to the package directory. `targets` is optional; when present, importing the package for any other target fails before target lowering. The built-in `RetroSharp.Portable2D` package lives in `sdk/RetroSharp.Portable2D` and is registered through the same manifest loader used by external packages. Source-only libraries still do not add new compiler semantics by themselves. When a host needs source package plus new semantic descriptors, it must pass an explicit in-process `SdkPluginRegistry`; that separate plugin level can provide namespaced resource/operation descriptors, validators, capability ids, and target lowering hooks without adding a new central enum member. Plugin capabilities are enforced per target (a lowering hook must grant an operation's required capabilities) and the RetroSharp CLI host-registers plugins through `--sdk-plugin <id>` or a manifest `plugins` array; the camera AABB queries below stay built-in Portable2D until a non-breaking migration moves them to a plugin. This MVP does not yet model package versions, dependencies between packages, remote package feeds, binary libraries, per-library asset roots, dynamic plugin loading, or binary backend plugins. Runtime helpers such as `Video.WaitVBlank()`, `Input.Poll()`, `Audio.Update()`, `Music.Play(...)`, `Sfx.Play(...)`, `Camera.SetPosition(...)`, camera AABB queries, `Sprite.Draw(...)`, and `World.TileFlagsAt(...)` are ordinary package source over target intrinsics. Resource declarations such as `Sprite.Asset(...)`, `World.Load(...)`, `Music.Asset(...)`, `Sfx.Asset(...)`, `Palette.*(...)`, and `Animation.Clip(...)` are ordinary package facades marked with `[resource(...)]`; the frontend resolves that metadata through registry-backed resource declaration descriptors, then the active target performs the existing asset import, variant resolution, palette validation, animation clip validation, and audio asset validation.
...
| `button` argument | A `Button` enum member (`Button.A`, `Button.Right`, ...). Bare lowercase button identifiers and direct snake_case button calls are rejected at the public source layer. |

Supported logical buttons are `Button.A`, `Button.B`, `Button.Select`, `Button.Start`, `Button.Right`, `Button.Left`, `Button.Up`, and `Button.Down`.
...
| `Audio.Update()` | Advance the target audio runtime once. Call it once per frame after the frame boundary for targets whose audio runtime is tick-driven. Game Boy advances queued BGM and active SFX streams here, applying SFX after BGM for the same frame; SFX play on channel 1 with BGM-priority ducking (the BGM's channel 1 writes are suppressed while an effect is active). NES advances BGM here and then ticks one frame of any active pulse 1 sound effect, suppressing the BGM's own pulse 1 writes while an effect owns the channel. |
...
Game Boy currently accepts VGM/VGZ DMG register logs, hUGETracker `.uge` v6 resources, and transitional `.gbapu` APU traces (binary, or legacy `retrosharp.gbapu.v1` `.gbapu.json`) for BGM directly or through the envelope. Game Boy SFX v1 accepts VGM/VGZ DMG register logs and reuses the compact APU-trace repack as a one-shot stream, filtered to channel 1 (`$FF10-$FF14`, the square+sweep channel) so it drops the globals `NR50/NR51/NR52` (incl. the `NR52` power-off) and other channels and cannot corrupt the BGM; the BGM ducks its own channel 1 while an effect plays and restores `NR10` (sweep) when it ends. The CLI has a target-specific `gbs-to-gbapu` export helper for preserving GBS APU register writes as `.gbapu`; `.gbs` is not a portable asset format. NES accepts VGM/VGZ 2A03 register logs for BGM and SFX; BGM can include pulse, triangle, noise, DMC/DPCM, `$4015`, and `$4017`, while the SFX path uses pulse 1 as the dedicated effect channel: it keeps only the pulse 1 registers (`$4000-$4003`), drops captured global/other-channel writes such as `$4010`, `$4015`, and `$4017`, and packs a flat per-frame one-shot trace played one frame per `Audio.Update()`. Expansion audio remains out of scope for v1. See `GameBoyApuTraceFormat.md` and `NesApuTraceFormat.md` for target-specific compiled trace details.
...
NES PNG lowering keeps the portable sprite logical: when more than three opaque source colors are present, the NES target keeps the three most representative colors in the base metasprite layer and emits remaining colors as optional transparent overlay pieces. The source still calls one `Sprite.Draw(...)`; the target asset compiler owns the extra OAM pieces, derived overlay palette, and diagnostics when the requested logical base slot leaves no overlay slot. When two PNG assets are drawn with the same logical slot but derive incompatible NES palettes, the NES backend may place one asset in a free physical sprite palette range while preserving the source-level logical slot.
...
| `Enemies.Def(name, sprite: asset, behavior: Behavior, animation: clip, speed: n, hp: n, cooldown: n, contactDamage: n, hitboxWidth: n, hitboxHeight: n, defeatedFrame: n)` | Declare byte-sized per-enemy metadata. `behavior`, `sprite`, and `animation` must be identifiers when supplied; numeric properties must be literal bytes. Omitted numeric properties default to `0`, except `hp`, which defaults to `1`; omitted `defeatedFrame` leaves drawing and update behavior unchanged. When supplied, actor drawing selects that literal sprite frame while the active actor's `health` is `0`, player-contact helpers ignore the defeated actor so they cannot retrigger its contact state, and `pool.Update()` stops advancing its behavior and animation until the slot is released. |
...
| `pool.TouchPlayer(playerX, playerY, playerWidth, playerHeight)` | Read the current camera X/Y once for the helper's generated loop, loop active slots, compute each actor's camera-relative `screenX`/`screenY`, branch by kind, cull slots outside the visible camera window, and test each kind's literal hitbox against a player AABB in screen coordinates. Player X/Y may be runtime expressions; width/height remain literal bytes. On hit, set actor `state` to `contactDamage` or `1`. |
| `pool.TouchPlayerTop(playerX, playerY, playerWidth, playerHeight, topDepth)` | Perform the same camera-relative player contact pass, but accept only horizontal overlap whose player bottom lies inside the actor's top band. `topDepth` is a literal byte from `1` to `255`. On contact, set actor `state` to `contactDamage` or `1`; the game remains responsible for requiring downward player motion and applying its bounce or damage policy. |
| `pool.DrawAndTouchPlayerTop(playerX, playerY, playerWidth, playerHeight, topDepth)` | Fuse `pool.Draw()` with the same top-band contact test. Each slot computes its camera projection once and shares the active/visible guard between the stable sprite call and contact, which is useful in fixed-PRG games that perform both operations in the same phase. It has the same literal/runtime operand rules and writes the same actor `state` result as `TouchPlayerTop`. |
...
`pool.TouchTiles(...)`, `pool.LandOnTiles(...)`, `pool.TouchPlayer(...)`, and
`pool.TouchPlayerTop(...)` use the same per-phase camera cache and per-actor
...
explicit `pool.DrawAndTouchPlayerTop(...)` combined phase is the exception: it
shares one projection and active/visible guard while retaining the same stable
sprite submission and contact result. The
...
For logical sprites, targets feed their compiled metasprite geometry and hardware sprite size mode into the same frame-budget pass. The compiler rejects unsupported `SpriteSizeModes`, one possible frame that exceeds `SpriteCount`, and `MaxSpritesPerScanline` when the sprite draw uses a constant Y position. NES automatic PNG overlay pieces count as real OAM pieces for total sprite usage, but scanline-pressure diagnostics treat them as optional detail so the base sprite remains the hard budget. Actor pool draws receive an earlier target-aware diagnostic after sprite assets are compiled: for every drawn pool, `capacity * max(Enemies.Def mandatory metasprite hardware sprites)` must fit `SpriteCount`, and `capacity * max(Enemies.Def busiest mandatory scanline hardware sprites)` must fit `MaxSpritesPerScanline`. That diagnostic names the pool and the enemy definition with the largest metasprite pressure. The frame-budget pass still validates the lowered draw operations with branch-as-alternative and frame-boundary semantics. Other runtime Y positions still cannot be placed on a specific scanline statically, so dynamic per-scanline overflow remains author/runtime responsibility outside the actor pool path. An unsound count over the flattened operation list would reject valid programs and is intentionally avoided. See issue #102.
...
| World collision queries | Supported on Game Boy runner path. | Generic `World.TileFlagsAt(...)` and `collision_aabb_tiles(...)` are not implemented in the current NES spike. |
...
| NES automatic sprite overlay palette overflow | `NES sprite asset 'hero' needs sprite palette slot 4 for an automatic PNG overlay, but target 'nes' supports slots 0..3.` |
...
The sample uses a target-variant JSON sprite asset (`platforms.gb` and `platforms.nes`). That asset format is transitional, but the source-level `Sprite.Asset(...)` and `Sprite.Draw(...)` calls are the portable SDK surface. Game Boy and NES currently lower `Sprite.Draw(...)` through the `RetroSharp.Portable2D` package helper.
