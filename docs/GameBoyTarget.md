...
- Projectile framework MVP: `Projectiles.Pool(...)`, `Projectiles.Def(...)`, `pool.Request(...)`, `pool.ProcessRequests()`, `pool.Update()`, `pool.Draw()`, `pool.TouchActors(...)`, and `pool.TouchHero(...)` expand before Game Boy lowering to fixed hero/enemy `Projectile` arrays, a fixed `ProjectileSpawnRequest` queue, inactive-slot initialization, literal metadata constants, deterministic queue/pool-full handling, lifetime-based despawn, camera-relative `Sprite.Draw(...)`, and actor/hero hitbox hooks; tile collision, offscreen-margin culling, particles/effects, sounds, and actor-emitter helpers remain follow-up work
