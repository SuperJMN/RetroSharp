...
- Actor framework pool/definition sugar: `actor.Pool(...)`, `actor.SpawnLayer(...)`, `actor.SpawnWindow(...)`, `enemy.Def(...)`, and `enemy.*` metadata helpers expand before Game Boy lowering to fixed `Actor` struct arrays, constants, inline helper branches, generated ROM-table spawn helpers, a fixed `used[]` activation byte array, and runtime camera-window activation loops from Tiled object layers; actor world X uses byte split storage (`x` low byte plus `xHi` high byte), while `y` remains byte-sized; `pool.Update()`/`pool.Draw()` support the basic byte-field behavior set (`Walker`, `Flyer`, `Patrol`, `Shooter`, `Hazard`, and direction-driven `Chaser`); optional `animation` metadata draws through `animation.Frame(...)`; actor draw computes `screenX = worldX - cameraX`, culls actors outside the visible camera window, and then calls `sprite.Draw`; `pool.TouchTiles(...)`/`pool.LandOnTiles(...)` use that same per-actor projection and lower through camera-relative AABB SDK calls, while `pool.TouchPlayer(...)` compares the projected actor AABB against a literal player AABB in screen coordinates; pool capacity is checked against the Game Boy 40-sprite target cap and against the maximum simultaneously activatable spawns in each declared camera window before lowering, and actor draw usage participates in the shared frame-budget validation
...
- `camera.SetPosition(x, 0)` advances the runtime camera by at most one pixel per call toward the requested low byte, using a signed 8-bit delta so the source-level `x` can wrap. A run frame that moves two pixels therefore calls `camera.SetPosition` once per single-pixel step (inside the per-pixel move) instead of once at the end of the frame; otherwise the camera falls a pixel behind each frame until the delta crosses 128 and the camera scrolls backward.
