...
See `ArchitectureOverview.md` for the layer model that separates the RetroSharp language, portable 2D SDK, and target intrinsics (roadmap iterations and the `AR-x.y` backlog live in `ArchitectureRoadmap.md`), and `Portable2DSdkV1.md` for the current SDK v1 reference. This file tracks the current Game Boy target subset and runner milestones.
...
Projects load `RetroSharp.Portable2D` from manifest `libraries`; standalone files can use `import RetroSharp.Portable2D;` as the explicit source-level form. Unknown imports fail compilation, and SDK dot-calls require a loaded source package. `Video.WaitVBlank()` and `Input.Poll()` are provided by that SDK source library as inline wrappers over Game Boy target intrinsics (`wait_frame`/`wait_vblank` and `poll_input`). `Audio.Init()`, `Audio.Update()`, `Music.Play(name)`, `Music.Stop()`, and `Sfx.Play(name)` are likewise provided by the SDK library over the `audio_init`/`audio_update`/`music_play`/`music_stop`/`sfx_play` target intrinsics (`music_play` and `sfx_play` carry the audio asset as a compile-time `AssetRef` operand), and `Music.Asset(...)` / `Sfx.Asset(...)` are package resource declarations. The collector still records them as `Sdk2DOperation.WaitFrame`/`Sdk2DOperation.PollInput` and the matching `SdkAudioOperation` values, so byte emission and frame-budget boundaries remain identical to the older direct SDK operation path. Logical sprite draw, explicit map-column streaming, camera movement, world-flag reads, and camera-relative AABB collision lower through one production `GameBoySdkOperationLowerer` instance whose feature partials own the target bytes. The runtime compiler uses `Sdk2DStreamReader` and `SdkAudioStreamReader` over `SdkProgram.Main`/`SdkAudioProgram.Main` plus their named subroutine streams for migrated statement and value calls, routes collected operations through that lowerer instead of reconstructing them from the AST, and fails if a stream and its source call sites diverge or remains unconsumed. `GameBoySdkLoweringArchitectureTests` checks that boundary from compiled operation symbols and call edges, so adding or renaming a feature partial does not change the ownership contract. Raw and transitional target calls reuse lowerer-owned internal emission where applicable without becoming new portable operations. Game Boy compilation also runs the shared frame-budget pass, so multiple explicit map-column streams that exceed the 21-tile background write budget, more than 40 hardware sprites, or more than 10 constant-Y sprites on a scanline in one possible frame fail before lowering.

Logical `Sprite.Draw(...)` programs use retained OAM: draw call sites write the
160-byte shadow at `$C600`, and `Video.WaitVBlank()` publishes that page with
one `$FF46` DMA. The DMA helper is copied to `$FF80-$FF89`, reserves the
remaining HRAM for the stack, and keeps instruction fetches outside the blocked
bus for the complete transfer. Packed camera commits that already entered the
current VBlank do not create a second publication boundary. Raw compatibility
operations such as `Sprite.Set(...)` retain their direct physical-OAM behavior.
...
- Actor framework pool/definition sugar: `Actors.Pool(...)`, `Actors.SpawnLayer(...)`, `Actors.SpawnWindow(...)`, `Enemies.Def(...)`, and called `Enemies.*` metadata helpers expand before Game Boy lowering to fixed `Actor` struct arrays, constants, generated columnar ROM-table spawn helpers, a fixed `used[]` activation byte array, and runtime camera-window activation loops from Tiled object layers; every varying spawn field retains authored order in one byte-per-record ROM column, while a field that is uniform across the layer remains a direct constant; unused `Enemies.*` lookup helpers are not generated and have zero byte cost; actor world X/Y use byte split storage (`x`/`y` low bytes plus `xHi`/`yHi` high bytes); `pool.Update()`/`pool.Draw()` support the basic byte-field behavior set (`Walker`, `Flyer`, `Patrol`, `Shooter`, `Hazard`, and direction-driven `Chaser`); optional `animation` metadata draws through `Animation.Frame(...)`; actor draw reads camera X/Y once per generated draw loop and computes `screenX = worldX - cameraX` and `screenY = worldY - cameraY`; every pool size projects inactive or off-window actors to hidden coordinates through stable logical draw call sites, preventing stale visible actor frames while preserving fixed OAM order; `pool.TouchTiles(...)`/`pool.LandOnTiles(...)` use the same per-phase camera cache and per-actor 2-axis projection before lowering through screen-space camera AABB SDK calls, while `pool.TouchPlayer(...)` compares the projected actor AABB against a literal player AABB in screen coordinates; drawn pool sprite pressure is checked as pool capacity times the largest target-resolved enemy metasprite against the Game Boy 40-sprite frame cap and 10-sprite scanline cap, spawn windows are checked against maximum simultaneously activatable spawns, and actor draw usage also participates in the shared frame-budget validation

Game Boy reads a fixed-bank spawn column with `LD A,(index); LD E,A; LD D,0;
LD HL,table; ADD HL,DE; LD A,(HL)`, a fixed 56-cycle path including the runtime
index load. When cartridge layout moves the immutable column to switchable ROM,
the same call uses the target's bank-preserving read-only-data helper and never
...
### MBC1 fixed-bank read foundation

Banked cartridges keep two deliberately different target-private bank values. `$C11C` is the executable program-bank continuation state. `$C1FA` is the authoritative shadow of the bank actually visible in `$4000-$7FFF`; every generated MBC1 `$2000` write updates `$C1FA` first. Every MBC1 layout initializes the hardware and `$C1FA` to bank 1, including layouts banked only for data/audio; `$C11C` is initialized only when executable code has a switchable program tail. Fixed-bank far reads therefore save `$C1FA` on the hardware stack and restore that actual entry bank in LIFO order rather than assuming `$C11C` describes an arbitrary reader entry.
...
`GameBoyRuntimeMemoryLayout` is the sole owner of the target-private WRAM contract: user locals `$C000-$C0DF`, runtime state `$C0E0-$C1EF` (including packed camera slot tags, visible-camera state, counters, visual-cache metadata, diagonal arbitration, packed-audio state, and collision query coordinates), fixed WorldPack scratch plus collision cache/tag and validation state `$C1F0-$C20F`, channel-1 audio shadow `$C210-$C214`, an optional 64-entry collision memo table at `$C220-$C2DF`, and `WorldPack` staging `$C300-$C529`. A direct one-byte-ID reader uses the original two visual slots and 298 staging bytes; the standard packed camera uses three visual slots and 362 bytes; a diagonal packed camera uses six visual slots and the complete 554-byte v1 maximum. Two-byte IDs also require the 554-byte maximum, and a 555-byte request is rejected. The memo table is selected only for the complete-stage 2x2-metatile geometry; it is outside staging and does not change those staging totals. These ranges are disjoint from WRAM echo and the `$FF80-$FFFF` stack/HRAM region. `GameBoyRuntimeMemoryLayoutTests` guards the named ranges, disjointness, and capacity boundaries.
...
camera builds. Small worlds that remain within the legacy byte/32-row limits
keep the raw compatibility path; the complete shared runner exceeds those
limits and therefore uses packed staging.
...
Full normalized `stage1` is exactly 2,609 bytes (60 chunks, 770 stored visual
bytes and 367 stored collision bytes)
...
`--target gb --world-budget-report <map.tmx|map.tmj>`. It emits deterministic JSON to
...
`Sprite.Asset(name, path, frameWidth, frameHeight)` loads an editable PNG sprite sheet relative to the `.rs` file. Frames are laid out horizontally, which maps directly to a simple Aseprite export. Transparent pixels become Game Boy sprite color `0`; each opaque source color is quantized from its original RGB luminance to sprite color `1` (light), `2` (middle), or `3` (dark), with at most three distinct opaque colors per sheet. This applies equally to RGB, RGBA, and indexed PNGs, so palette order and first-pixel order do not change the result. The sample palette maps `#E0F8D0` to `1`, `#88C070` to `2`, and `#346856` to `3`; grayscale exports likewise map light, middle, and dark values to `1`, `2`, and `3`.
...
framework backlog have moved to
[`history/GameBoyTargetMilestones.md`](history/GameBoyTargetMilestones.md).
They are background only; this document remains the current Game Boy subset.
