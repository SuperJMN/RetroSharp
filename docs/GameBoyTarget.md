...
The preferred source spelling is `import RetroSharp.Portable2D;` followed by SDK dot-calls such as `Video.Init()` and `Camera.SetPosition(x, y)`. The older snake_case function names remain accepted as compatibility aliases and lower through the same target path.
