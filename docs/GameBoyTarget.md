...
- Projectile/effect framework MVP: `Projectiles.Pool(...)`, `Projectiles.Def(...)`, `pool.Request(...)`, `pool.ProcessRequests()`, `pool.Update()`, `pool.Draw()`, `pool.TouchActors(...)`, `pool.TouchHero(...)`, `Effects.Pool(...)`, `Effects.Def(...)`, and `effects.*` lifecycle calls expand before Game Boy lowering to fixed hero/enemy `Projectile` arrays, fixed visual `Effect` arrays, fixed request queues, inactive-slot initialization, literal metadata constants, deterministic queue/pool-full handling, velocity-based projectile movement, camera-margin projectile culling, page-aware projectile-vs-actor collision, accumulated hero damage, optional projectile spawn/impact/expiration effect requests, and camera-relative `Sprite.Draw(...)`; tile collision, sounds, richer effect behavior, and actor-emitter helpers remain follow-up work
