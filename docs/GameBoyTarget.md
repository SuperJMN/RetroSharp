...
- Actor framework pool/definition sugar: `actor.Pool(...)`, `actor.SpawnLayer(...)`, `actor.SpawnWindow(...)`, `enemy.Def(...)`, and `enemy.*` metadata helpers expand before Game Boy lowering to fixed `Actor` struct arrays, constants, inline helper branches, and fixed slot stores from Tiled object layers; `pool.Update()`/`pool.Draw()` support the basic byte-field behavior set (`Walker`, `Flyer`, `Patrol`, `Shooter`, `Hazard`, and direction-driven `Chaser`); optional `animation` metadata draws through `animation.Frame(...)`; `pool.TouchTiles(...)`/`pool.LandOnTiles(...)` lower through camera-relative AABB SDK calls, and `pool.TouchPlayer(...)` lowers to literal AABB comparisons; pool capacity is checked against the Game Boy 40-sprite target cap before lowering, and actor draw usage participates in the shared frame-budget validation
