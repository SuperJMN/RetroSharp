...
- Actor framework pool/definition sugar: `actor.Pool(...)`, `actor.SpawnLayer(...)`, `actor.SpawnWindow(...)`, `enemy.Def(...)`, and `enemy.*` metadata helpers expand before Game Boy lowering to fixed `Actor` struct arrays, constants, inline helper branches, and fixed slot stores from Tiled object layers; actor world X uses byte split storage (`x` low byte plus `xHi` high byte), while `y` remains byte-sized; `pool.Update()`/`pool.Draw()` support the basic byte-field behavior set (`Walker`, `Flyer`, `Patrol`, `Shooter`, `Hazard`, and direction-driven `Chaser`); optional `animation` metadata draws through `animation.Frame(...)`; actor draw computes `screenX = worldX - cameraX`, culls actors outside the visible camera window, and then calls `sprite.Draw`; `pool.TouchTiles(...)`/`pool.LandOnTiles(...)` use that same per-actor projection and lower through camera-relative AABB SDK calls, while `pool.TouchPlayer(...)` compares the projected actor AABB against a literal player AABB in screen coordinates; pool capacity is checked against the Game Boy 40-sprite target cap before lowering, and actor draw usage participates in the shared frame-budget validation
