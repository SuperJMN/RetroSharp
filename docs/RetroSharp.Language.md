...
- Relational comparisons (`<`, `<=`, `>`, `>=`) on the current cartridge targets are **signed** when either operand is an `i8` value (local or byte-backed struct field), and **unsigned** otherwise. This lets `i8` hold two's-complement values so `velocity < 0` and `velocity > 0` work directly, while `u8`/`u16`/`i16` positions keep unsigned ordering for the 0..255 range the samples use. Signed lowering is zero-cost: both operands' sign bits are flipped (`XOR/EOR $80`) before the same unsigned compare/branch. Equality (`==`, `!=`) is unaffected by signedness. Signed comparison is currently detected for identifier and `base.field` operands; runtime-indexed struct-array fields such as `enemies[i].vx` still compare unsigned.
