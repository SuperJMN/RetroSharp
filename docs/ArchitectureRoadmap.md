...
Separate design debts remain valid outside that stabilization slice: #103 tracks language/SDK dot-call and receiver-lowering boundaries, #104 tracks type-system soundness, and #105 tracks the remaining Tiled import/world-flattening coupling. As of the latest pass, the dot-call precedence for #103 is now single-sourced: `RetroSharp.Parser.SdkDotCallResolver` owns the canonical rule (a receiver in scope shadows a same-named SDK module; otherwise a known SDK module resolves as an SDK call), and both constant folding and semantic analysis consume it. The residual is that constant folding has no variable scope, so it detects receivers by method signature; unifying the pathological collision case (a receiver method named like an SDK method on a bare module) still needs scope in the folder. For #105, the structural half is extracted: `RetroSharp.Core.Sdk.Tiled.LogicalTiledMapImporter` now owns target-neutral Tiled parsing, tileset descriptors, geometry/world-slice resolution, and collision-flag interpretation, producing a `LogicalTiledMap` of source-tile references. The Game Boy importer consumes it and keeps only pixel generation, deduplication, 8x8 expansion, and per-pixel background composition. The remaining coupling is that `WorldMap2D` still stores already-lowered target tile ids and NES does not consume the neutral map yet.
...
| `camera_aabb_tiles(...)` | Portable SDK capability-gated query | Camera-relative AABB bridge for fixed-screen actors on long maps; requires target support for `CameraRelativeAabb`. |
...
Status: first slice landed; not complete. The language/storage prerequisite (byte-sized struct arrays with `arr[i].field` access), actor pool/definition frontend, Game Boy/NES basic behavior `Update`/`Draw`, actor animation/camera-AABB helpers, Tiled object-layer spawn with literal (compile-time) window activation, player-contact helper coverage, conservative actor scanline budgeting, and target pool/sprite-count diagnostics landed on branch `feature/actor-framework`. Phase 5 has started: actor positions now use world-space X split as `x` low byte plus `xHi`, and draw is camera-relative with offscreen culling. Remaining review findings are still open: collision uses one fixed screen column, activation is compile-time filtering (not runtime paging), and capability checks count one actor as one hardware sprite. Those limitations are tracked as Phase 5 in `docs/ActorFrameworkRoadmap.md`, which also holds the full AR-14 task breakdown.
