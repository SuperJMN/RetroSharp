...
The IR stays at the immediate-value-or-storage-location level without gaining general source syntax trees. Both targets retain a flattened operation list for capability validation and consume a reachability-aligned `Sdk2DProgram` with separate main and named-subroutine streams during runtime emission. Target-owned stream readers handle migrated statement and value calls, and both targets route camera-AABB value operations through the same lowerer instance used for statement operations. The stream readers fail if a source call and the next collected operation disagree, or if collected operations remain after emission.
