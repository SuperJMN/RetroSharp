...
The active layer model, capability model, SDK operation model, golden rule,
shared world-map resource, and agent task contract now live in
[`ArchitectureOverview.md`](ArchitectureOverview.md). This file keeps the
roadmap iterations and the `AR-x.y` implementation backlog that
`tools/roadmap/seed_github_issues.py` consumes.
