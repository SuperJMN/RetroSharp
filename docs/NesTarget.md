...
The NES target compiles the constrained subset to mapper 0 when that exact image fits, then automatically retries the internal fixed-execution mapper 4 / TVROM-style four-screen profile when the final PRG/DPCM link needs it. If that second attempt proves that the flattened gameplay stream alone causes the fixed PRG overflow, the linker may select `nes-mmc3-tvrom-codebank-v1` and place that stream in unused whole R6 banks. It supports tick-based input, logical sprites, staged packed-world row/column streaming beyond the preloaded 64x60 surface, runtime animation helpers, camera-relative collision queries, VGM/VGZ-sourced 2A03 BGM including DMC/DPCM, and the actor-framework acceptance slice. The shared runner loads complete `stage1.tmx`; its final image selects MMC3 automatically while smaller packed worlds may still fit mapper 0. NES still does not support HUD, generic world-space collision queries, expansion audio, runtime CHR banking, or public mapper APIs. See [`NesCodeBankingV1.md`](NesCodeBankingV1.md) for executable placement and `docs/history/LargeWorldsRoadmap.md` for the completed cross-target packed-world and mapper execution plan.
