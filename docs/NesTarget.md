...
Logical sprite programs use retained OAM page `$0200`. Each
`Video.WaitVBlank()` publishes that page once through `$4014` while rendering is
in VBlank, then resets only the statically allocated logical call-site bytes to
`$FF` before the next frame draws them. An unexecuted conditional draw therefore
becomes canonical unused OAM instead of retaining an earlier sprite. Startup
still initializes the full page, while raw/direct compatibility programs keep
their existing `$2003/$2004` path and do not acquire an extra DMA.
