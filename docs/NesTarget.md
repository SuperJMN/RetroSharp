...
`camera.AabbTiles(screenX, worldY, width, height, flags)` and `camera.AabbHitTop(screenX, worldY, width, height, flags)` query collision flags from the active world map using the current camera X position plus a fixed screen-space actor rectangle. They support the runner's fixed-screen actor shape on long horizontal maps. Generic world-space `world_tile_flags_at(...)` and `collision_aabb_tiles(...)` remain unsupported on NES.
