...
The NES target compiles the constrained subset to mapper 0 when that exact image fits, then automatically retries the internal fixed-execution mapper 4 / TVROM-style four-screen profile when the final PRG/DPCM link needs it. If that second attempt proves that the flattened gameplay stream alone causes the fixed PRG overflow, the linker may select `nes-mmc3-tvrom-codebank-v1` and place that stream in unused whole R6 banks. When the R6 pool itself runs out, it retries on the next larger MMC3 board (128, 256, or 512 KiB) without any author-facing selector. It supports tick-based input, logical sprites, staged packed-world row/column streaming beyond the preloaded 64x60 surface, runtime animation helpers, camera-relative collision queries, VGM/VGZ-sourced 2A03 BGM including DMC/DPCM, and the actor-framework acceptance slice. The shared runner loads complete `stage1.tmx`; its final image selects MMC3 automatically while smaller packed worlds may still fit mapper 0. NES still does not support HUD, generic world-space collision queries, expansion audio, runtime CHR banking, or public mapper APIs. See [`NesCodeBankingV1.md`](NesCodeBankingV1.md) for executable placement and `docs/history/LargeWorldsRoadmap.md` for the completed cross-target packed-world and mapper execution plan.
...
The target-private `NesFramePlan` is selected before emission and immediately
encapsulated by `NesPhysicalFrameScheduler`, the single executable authority
for runtime NMI/VBlank admission, retained OAM publication, explicit video-safe
...
`NesSdkOperationLowerer*`, `NesSdkStreamReader`, `PrgBuilder`, and
`NesPrgLinker`
...
`Camera.Init(mapWidth, streamY, streamHeight)` enables the camera path for the current world map. `Camera.SetPosition(x, y)` consumes complete little-endian word operands, stores the packed Y destination in target-private ABI bytes, and walks toward both destinations by at most one tile crossing (≤8 px) per axis per call. Packed builds keep requested/logical coordinates separate from visible coordinates: fine movement publishes immediately only when it needs no new edge, while a tile crossing publishes after its correctly tagged resident edge completes. The fixed, bank-neutral NMI increments only a 16-bit hardware-frame counter and sets a saturated pending-frame byte. Both mapper-0 and MMC3 packed-camera links vector NMI to that fixed handler; mapper-0 reset and IRQ retain their simple `$8000` targets, and mapper-0 programs without the packed camera keep all three vectors at `$8000`. If `Video.WaitVBlank()` enters with that signal already set, it consumes the signal and resumes gameplay/audio without publishing; it suppresses the following explicit `Camera.Apply()` and defers `Camera.SetPosition()`'s visible-state publication while retaining its logical/preparation work. A path that starts without a signal waits for a new NMI and rechecks hardware `$2002` before committing one bounded phase, restoring visible scroll, and publishing OAM. A late main loop therefore neither loses a physical gameplay/audio tick nor exposes a camera edge or OAM partway through VBlank. Request, resident, and commit stamps plus tile/attribute and forbidden-work counters make the two-frame preparation and zero-bank/directory/decode-in-commit contracts directly testable. The mapper-0 runtime evidence is recorded in [`NesMapper0PackedCameraAcceptance.md`](history/NesMapper0PackedCameraAcceptance.md), the measured complete-stage cadence and boundary evidence is recorded in [`NesRunnerCadenceAcceptance.md`](history/NesRunnerCadenceAcceptance.md), and the mixed-load phase sweep is recorded in [`AudioMixedLoadFunctionalAcceptance.md`](history/AudioMixedLoadFunctionalAcceptance.md). The retired AprNes/FCEUmm/Nestopia differential survives only as historical evidence in [`NesRunnerVisualParityAcceptance.md`](history/NesRunnerVisualParityAcceptance.md).
...
and that epic does not implement executable-code banking.

The later target-private [`NES Code Banking v1`](NesCodeBankingV1.md)
extension preserves the Large Worlds data-only profile and adds a distinct
final-link result only when its fixed gameplay stream does not fit. It reserves
`WorldPack` banks first, assigns the remaining whole R6 banks to flattened
`Main` code, and keeps startup, target subroutines, DPCM, handlers, reset,
banking helpers, and vectors fixed. Fixed veneers own cross-bank jumps and
fallthrough while preserving A and status and updating the R6 shadow; fixed
`WorldPack` helpers restore the caller's active code bank. No public source,
SDK, manifest, runtime-ABI, or mapper-selection surface is added.
...
[`NesRunnerPowerOnRamAcceptance.md`](history/NesRunnerPowerOnRamAcceptance.md).

NES packed-camera acceptance uses in-process gameplay simulation on a freshly
compiled ROM. An AprNes-only physical smoke run is available when PPU/OAM
timing needs independent diagnostic evidence, but it is not a closeout gate.
The former AprNes/FCEUmm/Nestopia comparison is retained only as the historical
#327 record in
...
One NES lookup is `LDA index; TAX; LDA table,X`, costing 9 CPU cycles or 10 on
an indexed page crossing regardless of whether the layer contains 16, 64, 128,
or 240 records. The 16-record/two-varying-column GCP-1.1 probe reduces fixed
payload from 2,656 to 2,366 bytes and adds no mutable per-record RAM.
...
Runner-shaped NES parity is covered by `NesRunnerAcceptanceTests`: the shared runner source must compile for NES with VGM audio, and runtime animation must lower. `CrossTargetScrollAcceptanceTests` also verifies that runner-shaped `Camera.AabbTiles(...)` and `Camera.AabbHitTop(...)` lower on both Game Boy and NES, that a bounded four-screen camera source lowers on NES, that `samples/tiled-vertical-scroll/vertical-scroll.rs` lowers to four-screen vertical NES scroll from a tall Tiled `World.Load(...)` map, and that `samples/tiled-cross-target-2d-scroll/cross-target-2d-scroll.rs` lowers to diagonal four-screen NES scroll from a wide/tall Tiled map with populated nametable attributes. The exact tracked-ROM production matrix for those samples plus `deadzone-follow` and the collision-free short/full horizontal `stage1` canaries is recorded in [`PackedTiledFunctionalAcceptance.md`](history/PackedTiledFunctionalAcceptance.md), including per-frame authored palette checks and external NesMcp/AprNes checkpoints.
