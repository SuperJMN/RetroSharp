...
internal static class GameBoyWorldPackRuntimeEmitter
{
    ...
    // many rows above the preparation-time top row and widened by twice that, so both endpoints
    // stay resident whichever way the camera drifts.
    internal const int DiagonalColumnRowSlack = 1;
        ...
        bool enablePackedAudioService)
    {
        ...
        if (enableStagedCamera)
        {
            ...
            EmitEdgePreparationRuntime(builder, plan, layout, enablePackedAudioService, enableDiagonalVisualCache);
        }
        ...
        bool diagonalStreaming)
    {
        ...
        if (layout.WorldPackColumnPlanePlacement is { } columnPlanePlacement)
        {
            ...
            var rowPlane = builder.CreateLabel("worldpack_row_plane_preparation");
            var columnPlaneBounds = builder.CreateLabel("worldpack_column_plane_bounds");
            var rowPlaneBounds = builder.CreateLabel("worldpack_row_plane_bounds");
            ...
            builder.JumpAbsolute(0xC2, rowPlane);
            ...
            builder.Emit(0x06, (byte)WorldPackRuntimeResult.BoundsError);
            builder.JumpAbsolute(failed);

            builder.Label(rowPlane);
            EmitJumpIfWordOutside(
                builder,
                GameBoyRuntimeMemoryLayout.PackedCamera.CommitWorldEdgeLow,
                GameBoyRuntimeMemoryLayout.PackedCamera.CommitWorldEdgeHigh,
        ...
        var rowLength = builder.CreateLabel("worldpack_edge_row_length");
        var lengthReady = builder.CreateLabel("worldpack_edge_length_ready");
        builder.JumpAbsolute(0xCA, rowLength);
        EmitLoadPackedColumnPayloadLength(builder, diagonalStreaming);
        ...
        builder.Emit(0x06, (byte)WorldPackRuntimeResult.Success);
        builder.Emit(0xC9); // RET

        builder.Label(noSlot);
        builder.LoadAImmediate(GameBoyPackedCameraRuntime.NoSlot);
        builder.Emit(0x06, (byte)WorldPackRuntimeResult.Miss);
        builder.Emit(0xC9); // RET
    }
        ...
        string success)
    {
        ...
        var maximumPayloadTiles = MaximumColumnPayloadTiles(diagonalStreaming);
        var lastNonWrappingStartRow = Math.Max(0, hardwareHeight - maximumPayloadTiles);
        ...
        if (hardwareHeight < maximumPayloadTiles)
        {
            builder.JumpAbsolute(wraps);
        }
        else
        {
            builder.LoadA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorHigh);
    ...
    private static void EmitRowPlaneCopy(
        GbBuilder builder,
        int hardwareWidth,
        int hardwareHeight,
        GameBoyWorldPackColumnPlanePlacement placement,
        ...
        string success)
    {
        ...
        EmitAdvanceRowPlaneIterator(builder, hardwareWidth);
        builder.LoadAFromE();
        builder.StoreA(GameBoyRuntimeMemoryLayout.PackedCamera.DestinationLow);
        builder.LoadAFromD();
        builder.StoreA(GameBoyRuntimeMemoryLayout.PackedCamera.DestinationHigh);
    ...
    internal static int MaximumColumnPayloadTiles(bool diagonalStreaming) =>
        diagonalStreaming ? PackedColumnPayloadTilesWithVerticalSlack : PackedColumnPayloadTiles;

    internal const int MaximumColumnPayloadTilesForDiagonal = PackedColumnPayloadTilesWithVerticalSlack;

    private static void EmitLoadPackedColumnPayloadLength(GbBuilder builder, bool diagonalStreaming) =>
        builder.LoadAImmediate(checked((byte)MaximumColumnPayloadTiles(diagonalStreaming)));

    private static void EmitAdvanceRowPlaneIterator(GbBuilder builder, int hardwareWidth)
    {
        var subtractWidth = builder.CreateLabel("worldpack_row_plane_iterator_subtract_width");
        var ready = builder.CreateLabel("worldpack_row_plane_iterator_ready");
        builder.LoadA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorLow);
        builder.AddAImmediate(21);
        builder.StoreA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorLow);
        builder.LoadA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorHigh);
        builder.AdcAImmediate(0);
        builder.StoreA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorHigh);
        builder.CompareImmediate(hardwareWidth >> 8);
        builder.JumpAbsolute(0xDA, ready);
        builder.JumpAbsolute(0xC2, subtractWidth);
        builder.LoadA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorLow);
        builder.CompareImmediate(hardwareWidth & 0xFF);
        builder.JumpAbsolute(0xDA, ready);
        builder.Label(subtractWidth);
        builder.LoadA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorLow);
        builder.SubtractAImmediate(hardwareWidth & 0xFF);
        builder.StoreA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorLow);
        builder.LoadA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorHigh);
        builder.SbcAImmediate(hardwareWidth >> 8);
        builder.StoreA(GameBoyRuntimeMemoryLayout.PackedCamera.IteratorHigh);
        builder.Label(ready);
    }
    ...
    private static void EmitEndEdgePreparationBankSession(GbBuilder builder)
    {
        ...
        builder.LoadAFromB();
        builder.Emit(0xF5); // PUSH AF; the wait and instrumentation must preserve the bank to restore.
        ...
        bool enableDiagonalVisualCache)
    {
        ...
        builder.LoadAImmediate(0);
        builder.Emit(0x06, (byte)WorldPackRuntimeResult.Malformed);
        builder.Emit(0xC9);
    ...
    int TotalBytes)
{
        ...
        bool enableCollisionMemoTable = false)
    {
        ...
        var cursor = (int)GameBoyRuntimeMemoryLayout.WorldPackStaging.Start;
        var visualSlots = WorldPackStagingLayoutAllocator.CreateSlots(
            (index, start, length) => new GameBoyWramRange($"WorldPack visual slot {index}", start, length),
            visualIdBytes * ChunkCells,
            visualSlotCount,
            ref cursor);
        var collisionSlots = WorldPackStagingLayoutAllocator.CreatePair(
            (index, start, length) => new GameBoyWramRange($"WorldPack collision slot {index}", start, length),
            collisionIdBytes * ChunkCells,
            ref cursor);
        var edgeSlots = WorldPackStagingLayoutAllocator.CreatePair(
            (index, start, length) => new GameBoyWramRange($"WorldPack edge slot {index}", start, length),
            EdgeBytes,
            ref cursor);
