...
The background uses five built-in Game Boy tiles: empty sky, cloud, distant hill, hazard spikes, and brick/ground. The editable level lives at `maps/runner.tmj` and can be opened in Tiled together with `maps/runner-tiles.tsj`. The `background` layer draws decorative clouds and hills, the `world` layer draws streamable terrain, and the hidden `collision` layer stores flags with `0` empty, `1` solid, `2` hazard, and `4` platform. The map custom properties `retrosharpStreamY`, `retrosharpWorldY`, and `retrosharpWorldHeight` select the rows imported into the active streaming world.
