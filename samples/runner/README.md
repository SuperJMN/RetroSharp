...
The background music is `music/delight.gbapu`, a Game Boy APU register trace used directly as a Game Boy BGM resource through `music.Asset(...)`, `audio.Init()`, `music.Play(...)`, and the per-frame `audio.Update()` call. NES validates those calls and lowers them as no-ops until NES BGM has its own implementation. The trace is derived from `music/free_06_delight.uge` (kept as the authoring source); both are from Tronimal's Free Game Boy Music Pack and licensed `CC-BY 2025`.
