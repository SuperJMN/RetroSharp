...
The starting area also contains one animated 16x16 Goomba. It enters from screen
X `176`, advances one pixel every two ticks toward the left, and uses one stable
`Sprite.Draw(...)` call that receives a hidden Y coordinate after it leaves the
screen. This first enemy deliberately has no combat, gravity, or patrol behavior.
...
The frame loop is intentionally narrow: it enters VBlank, applies at most one prepared camera edge, presents the player and Goomba, then ticks `Audio.Update()` and `Input.Poll()` once per frame. Game Boy keeps bank selection, directory lookup, and decode outside VBlank; the commit is only a resident 19-tile column, a 20-tile column that preserves a prefetched-row intersection, or a 21-tile row copy. Because the runner metasprites can consume the remaining OAM write budget, a packed-edge commit frame retains its previous OAM entries and refreshes them on the next frame, keeping audio/input/simulation at 60 Hz and all VRAM/OAM writes legal. On NES, `Video.WaitVBlank()` restores the pending four-screen camera state at NMI entry and the explicit `Camera.Apply()` remains the source-level guard. The gameplay helpers then handle gravity, collision, reset, input, camera request, and animation without hidden allocation or dispatch.

The runner keeps this one-enemy path deliberately smaller than the scalable
actor framework so it fits the current NES fixed PRG section. Actor pools,
Tiled spawn tables, collision/contact, and the built-in behavior matrix remain
covered by `samples/actor-framework/actors.rs` and the roadmap in
`docs/ActorFrameworkRoadmap.md`.
