...
The starting area also contains one animated 16x16 Goomba authored at world
position `(176, 288)` in the `actors` object layer of `stage1.tmx`. A one-slot
Actor Framework pool activates it through `Actors.SpawnWindow(...)`, and its
`Patrol` definition starts with `facing=1` so it advances one pixel every two
ticks toward the left. Its cooldown is longer than the trip out of the authored
camera window, so this first enemy does not reverse before despawning. The
