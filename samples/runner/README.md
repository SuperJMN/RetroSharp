...
The frame loop is intentionally narrow: `PresentFrame(...)` owns VBlank and writes the player OAM entries first; the loop then draws the enemy pool with `goombas.Draw()`, applies the camera with `camera.Apply()`, and ticks `audio.Update()` once per frame. Both the player OAM and the enemy OAM are written before `camera.Apply()` so every OAM write stays inside the VBlank window, while the camera's background-column/row streaming runs after the sprites. `frame.Begin()` clears transient collision state, `player.ApplyGravity()` advances physics, `view.FollowPlayer(player)` updates vertical dead-zone camera state, `view.CaptureScreen(player)` records the current projected screen position, `frame.ResolveSolidLanding(...)`, `frame.ResolveCeilingHit(...)`, `frame.ResolveFall(...)`, and `frame.ResolveReset(player, view)` handle collision outcomes, `player.HandleJumpInput()` and `view.HandleHorizontalInput(player, movementFootWorldY)` consume input and horizontal inertia, two `view.ApplyPosition()` calls synchronize the camera backend, and `player.UpdateRunAnimation(view)` derives the displayed frame. The helper boundaries are chosen around gameplay responsibilities, not around individual opcodes, so the source remains readable while still lowering to inline target code.
