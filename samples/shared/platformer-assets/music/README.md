...
`runner.gb.vgz` and `runner.nes.vgz` are stable copies used by focused canaries.
A shared source declares `runner.vgz`; the compiler resolves the `.gb.vgz` or
`.nes.vgz` variant for the selected target and repacks the VGM register writes
into the target's compact on-ROM stream. The live runner owns separate editable
copies under `samples/runner/assets/music`.
